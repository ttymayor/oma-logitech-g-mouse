...
function barTooltip(mouse) {
  if (!mouse || !mouse.connected) return "G PRO X2 SUPERSTRIKE (Disconnected)";
  var parts = ["G PRO X2 SUPERSTRIKE"];
  if (mouse.batteryPercentage !== LEVEL_UNKNOWN) {
    parts.push(
      batteryText(mouse.batteryPercentage) + " (" + mouse.batteryStatus + ")",
    );
  }
  if (mouse.dpiX > 0) {
